...
| `tasks[].feedback_from_checker` | string\\|null | v1.1: 代码审查反馈（code_review_failed 时有值）。v2: checker 审查意见（code_review_failed 或 verification-failed 时填充） |
...
while true:
  ...
  valid_tasks = [t for t in makers where t.verdict not in ("code_review_failed", "fail")]
  ...
  //   分别 apply + verify
  //      group_a 不通过 & group_b 通过 → culprit 在 group_a 中，递归二分
  //      group_b 不通过 & group_a 通过 → culprit 在 group_b 中，递归二分
  //      两者都不通过 → 无法隔离 → 全部回退
  //      两者都通过 → 确认语义兼容，无冲突
  // 定位到最小冲突对后：仅回退该对对应的 task，其余保留 completed
