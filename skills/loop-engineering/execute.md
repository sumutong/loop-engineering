...
| `tasks[].maker_result.verdict` | string | 验证判定：`pass` / `fail` / `undetermined` |
| `tasks[].maker_result.timestamp` | string | ISO 8601 时间戳，本轮 maker 完成时间 |
| `tasks[].feedback_from_checker` | string\|null | v1 始终为 null。v2 预留：checker 的审查意见 |
...
> maker Agent 执行期间父会话同步阻塞，无法刷新心跳。1800s 只覆盖"单次 maker"，cp 后的补心跳确保了这一点。
>
...
v1 checker 仅运行 verification 命令 + 解析输出，不做代码级审查。验证质量完全取决于 verification.command 的覆盖度。

**v1 内置硬编码检查（在 verification 之前运行）：** 检测测试文件删除（见执行流程 step 4(a)）。仅覆盖 JS/TS 测试文件（`*.test.*` `*.spec.*`）。
...
v2 的 `review_criteria` 字段将做代码级审查。
...
10. **checker 不做代码审查**——留给 v2 的 `review_criteria` 字段
